# Upgrade Windows 11 to Current Build
# Adapted from: https://github.com/IsaacGood/Upgrade-Windows-to-Current-Build-Powershell/
# Adapted for ConnectWise Automate: Write-Log/Get-TimeStamp helpers, ProgramData paths,
# Windows 10 upgrades disabled (handled separately).
#
...
#     - Make sure to increase the script timeout in Automate to longer than your
...
#       ScreenConnect Backstage, but works ok from Automate (usually).
#
# If you have issues upgrading, here are some things to check in order of commonality:
#     - The script output and any errors therein (also written to $LogPath).
...
# Upgrade Windows 10 to 11 - DISABLED, Windows 10 upgrades are handled separately
$UpgradeWindows10 = $false
...
$AppDir = "$env:ProgramData\Automation\Apps\WinUpgrade"
$TargetFolder = $AppDir

# Log file location
$LogPath = "$env:ProgramData\Automation\Logs\WinUpgrade_Upgrade.log"
...
function Get-Download {
    ...
    Write-Log "Downloading: $URL ($([math]::round($DownloadSize/1MB, 2)) MB) Destination: $TargetFolder\$FileName..."
...
function Invoke-CumulativeUpdate {
    ...
    if (((New-Object -ComObject 'Microsoft.Update.Session').QueryHistory("", 0, 1) | Where-Object { $_.Date -gt $BeginTime -and $_.Title -like "*$KB*" }).ResultCode -eq "4") {
        ...
        if ($minutes -eq $TimeToWaitForCompletion) {
            Exit-WithError "It's been over $TimeToWaitForCompletion minutes, something probably went wrong. Here's the Windows Update event logs: $((Get-EventLog -Log 'System' -Source 'Microsoft-Windows-WindowsUpdateClient' -After $BeginTime -ErrorAction SilentlyContinue).Message)"
        } else {
