...
# REVISED 10/24
# Redundant in Win 11, this folder no longer shown by default
...
# Cortana
# REVISED 10/24
# Cortana has largely been removed in Win 11, making this unnecessary
...
# REVISED 10/24
# Desktop shortcuts
# This makes sense to run after these are installed, but typically this script is run well before, I'm turning this into a scheduled task that will delete all public desktop shortcuts.
# Users can create their own shortcuts, or techs can create them during profile setup
...
# Edge browser
#REVISED 10/24 - Removed legacy Edge settings that don't apply to Chromium Edge
...
# Additionally, the sensorpermissions registry path is deprecated and doesn't exist in Win 11, this has been removed.
Write-Output "Enabling Location Services for automatic time zone detection... this will be disabled automatically within 24 hours"
If (!(Test-Path "HKLM:\SOFTWARE\Microsoft\Windows\CurrentVersion\CapabilityAccessManager\ConsentStore\location")) {
    New-Item -Path "HKLM:\SOFTWARE\Microsoft\Windows\CurrentVersion\CapabilityAccessManager\ConsentStore\location" -Force | Out-Null
}
Set-ItemProperty -Path "HKLM:\SOFTWARE\Microsoft\Windows\CurrentVersion\CapabilityAccessManager\ConsentStore\location" -Name "Value" -Type String -Value "Allow"

# This scheduled task can be used if the time zone is not being correctly updated
# Create a one-time scheduled task to disable location services and the location framework service after 24 hours
# $trigger = New-ScheduledTaskTrigger -Once -At ((Get-Date).AddHours(24))
# $taskAction = "Powershell.exe -NoProfile -WindowStyle Hidden -Command `"Set-ItemProperty -Path 'HKLM:\SOFTWARE\Microsoft\Windows\CurrentVersion\CapabilityAccessManager\ConsentStore\location' -Name 'Value' -Type String -Value 'Deny'; If (!(Test-Path 'HKLM:\SYSTEM\CurrentControlSet\Services\lfsvc\Service\Configuration')) { New-Item -Path 'HKLM:\SYSTEM\CurrentControlSet\Services\lfsvc\Service\Configuration' -Force | Out-Null }; New-ItemProperty -Path 'HKLM:\SYSTEM\CurrentControlSet\Services\lfsvc\Service\Configuration' -Name 'Status' -PropertyType DWord -Value 0 -Force | Out-Null; Start-Sleep -Seconds 5; schtasks /delete /tn 'DisableLocationServicesAndLocationFramework' /f`""
# $action = New-ScheduledTaskAction -Execute "Powershell.exe" -Argument $taskAction
# $principal = New-ScheduledTaskPrincipal -UserId "SYSTEM" -LogonType ServiceAccount -RunLevel Highest
...
# Write-Output "Scheduled task created to disable location services and the location framework 24 hours from now and will remove itself after running."

# Timezone
# REVISED 10/24 - Enabling automatic timezone detection as we have clients that are not in PST. Auto updates will stop working once location is disabled
...
# Deprecated in Win 11 # Set-ItemProperty -Path "HKLM:\SOFTWARE\Microsoft\Windows NT\CurrentVersion\Sensor\Overrides\{BFA794E4-F964-4FDB-90F6-51056BFE4B44}" -Name "SensorPermissionState" -Type DWord -Value 0
...
#REVISED 10/24
# This version of "News and Interests" does not exist in Windows 11. It now appears to be a widget.
...
# Teams
#REVISED 10/24 - Unsure if this is still needed
